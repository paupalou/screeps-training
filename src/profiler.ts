...
function AlreadyWrappedError(this: WrapperError) {
  this.name = 'AlreadyWrappedError';
  this.message = 'Error attempted to double wrap a function.';
  this.stack = ((new Error())).stack;
}
interface IProfiler { profiler: any; }

function setupProfiler() {
  depth = 0; // reset depth, this needs to be done each tick.
  (Game as IProfiler & Game).profiler = {
...
function setupMemory(profileType: string, duration: boolean | number, filter: any) {
  resetMemory();
  const disableTick = _.isBoolean(duration) ? duration : Game.time + duration;
  if (!Memory.profiler) {
    Memory.profiler = {
      map: {},
      totalTime: 0,
...
interface OriginalFunction {
  ...
  apply: (who: unknown, args: IArguments) => unknown;
}

function wrapFunction(name: string, originalFunction: OriginalFunction) {
  if (originalFunction.profilerWrapped) { throw new (AlreadyWrappedError as any)(); }
...
function profileObjectFunctions(object: any, label: string) {
  ...
  Object.getOwnPropertyNames(objectToWrap).forEach(functionName => {
    ...
    if (hasAccessor) {
      ...
      const profileDescriptor: Record<string, unknown> = {};

      if (descriptor.get) {
        const extendedLabelGet = `${extendedLabel}:get`;
        profileDescriptor.get = profileFunction(descriptor.get as unknown as OriginalFunction, extendedLabelGet);
      }

      if (descriptor.set) {
        const extendedLabelSet = `${extendedLabel}:set`;
        profileDescriptor.set = profileFunction(descriptor.set as unknown as OriginalFunction, extendedLabelSet);
      }
...
function profileFunction(fn: OriginalFunction, functionName: string) {
...
const Profiler = {
  printProfile() {
    console.log(Profiler.output(undefined));
  },
  ...
  output(passedOutputLengthLimit: number | undefined) {
